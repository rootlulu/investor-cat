...
TRANSLATE_RETRIES = 3
NO_RESULTS_ERROR_RE = re.compile(
    r"^(?P<source>[^:：]+)[:：]\s*最近(?P<days>\d+)天没有抓到 Google News RSS 新闻$"
)
...
async def get_news(refresh: bool = False, allow_stale: bool = True, force: bool = False) -> dict[str, Any]:
    ...
    translation_timeout = float(fetch_config.get("translation_timeout_seconds", 45))
    sources_with_domains = [source for source in enabled_sources if source.get("domains")]
    ...
    async with httpx.AsyncClient(
        ...
        tasks = [
            ...
            for source in sources_with_domains
        ]
    ...
    errors: list[str] = []
    warnings: list[str] = []
    for source, result in zip(sources_with_domains, results, strict=False):
        if isinstance(result, SourceNoResultsError):
            warnings.append(str(result))
        elif isinstance(result, Exception):
            errors.append(summarize_news_error(result, str(source.get("name") or "未知来源")))
        else:
    ...
    async with httpx.AsyncClient(
        ...
        translated_titles, translation_warnings = await translate_titles_bounded(
            candidate_items,
            translate_client,
            request_state,
    ...
    data["hasData"] = bool(data["china"] or data["world"])

    if not data["china"] and not data["world"] and stored:
        return build_stale_news_fallback(stored, errors=errors, warnings=warnings)
...
class SourceNoResultsError(RuntimeError):
    """A source responded successfully but had no matching items in the window."""
...
async def fetch_source(
    ...
    try:
        ...
        raise SourceNoResultsError(f"{source['name']}：近{request_state.days}天无匹配新闻")
    except SourceNoResultsError:
        raise
    except Exception as error:
        raise RuntimeError(summarize_news_error(error, str(source.get("name") or "未知来源"))) from error
...
async def fetch_google_news(
    ...
    if not successful_responses and failures:
        raise failures[0]
...
async def translate_titles_bounded(
    items: list[dict[str, Any]],
    client: httpx.AsyncClient,
    request_state: RequestState,
    *,
    ...
    try:
        translated = await asyncio.wait_for(
            translate_titles(items, client, request_state),
            timeout=max(0.01, timeout_seconds),
        )
        return translated, []
    except TimeoutError:
        return {}, ["标题翻译：超时，已使用本地标题"]
    except Exception:
        return {}, ["标题翻译：失败，已使用本地标题"]
...
def ensure_detail_fields(data: dict[str, Any]) -> dict[str, Any]:
    ...
    data["hasData"] = bool(data.get("china") or data.get("world"))
    return data


def normalize_news_issues(data: dict[str, Any]) -> dict[str, Any]:
    errors: list[str] = []
    warnings = [clean_text(str(item)) for item in data.get("warnings", []) if clean_text(str(item))]
    for item in data.get("errors", []):
        text = clean_text(str(item))
        if not text:
            continue
        no_results = NO_RESULTS_ERROR_RE.match(text)
        if no_results:
            warnings.append(
                f"{no_results.group('source').strip()}：近{no_results.group('days')}天无匹配新闻"
            )
            continue
        source, separator, detail = text.partition(":")
        if not separator:
            source, separator, detail = text.partition("：")
        if separator and source.strip() and detail.strip():
            errors.append(summarize_news_error(RuntimeError(detail.strip()), source.strip()))
        else:
            errors.append(summarize_news_error(RuntimeError(text), "新闻来源"))
    data["errors"] = list(dict.fromkeys(errors))
    data["warnings"] = list(dict.fromkeys(warnings))
    return data


def summarize_news_error(error: BaseException, source_name: str) -> str:
    source = clean_text(source_name) or "未知来源"
    if isinstance(error, TimeoutError):
        return f"{source}：抓取超时"

    detail = clean_text(str(error))
    detail = re.sub(rf"^{re.escape(source)}\s*[:：]\s*", "", detail, flags=re.I)
    lowered = detail.casefold()
    if detail in {"抓取超时", "请求超时"}:
        reason = detail
        ...
        reason = "请求限流（HTTP 429）"
    elif any(token in lowered for token in ("403", "waf", "captcha", "验证码", "风控")):
        reason = "访问受限"
    elif any(token in lowered for token in ("connecterror", "connection", "name resolution", "连接失败")):
        reason = "连接失败"
    else:
        status_match = re.search(r"\bHTTP\s*(\d{3})\b", detail, re.I)
        if status_match:
            reason = f"请求失败（HTTP {status_match.group(1)}）"
        else:
            reason = re.sub(r"^news\.google\.com\s+请求失败\s*[:：]\s*", "", detail, flags=re.I)
            reason = limit_text(reason or type(error).__name__, 48)
    return f"{source}：{reason}"


def build_stale_news_fallback(
    stored: dict[str, Any],
    *,
    errors: list[str],
    warnings: list[str],
) -> dict[str, Any]:
    fallback = ensure_detail_fields(dict(stored))
    fallback["cached"] = True
    fallback["fromStorage"] = True
    fallback["stale"] = True
    fallback["errors"] = list(errors)
    fallback["warnings"] = list(warnings)
    normalize_news_issues(fallback)
    fallback["hasData"] = bool(fallback.get("china") or fallback.get("world"))
    return fallback
