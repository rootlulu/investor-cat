...
SMM_LME_LIVE_URL = "https://news.smm.cn/live/metal/130"
SMM_LME_INVENTORY_API_URL = "https://news.smm.cn/api/list/metals/130/types/94/page/{page}/limit/30"
...
INVENTORY_MARKET_LABELS = {
    "domestic": "国内",
    "international": "海外",
}
LME_INVENTORY_METALS = {
    "copper": "铜",
    "aluminum": "铝",
    "zinc": "锌",
    "tin": "锡",
...
async def fetch_commodity_sources(client: httpx.AsyncClient) -> list[tuple[Any, str]]:
    ...
    return await asyncio.gather(
        ...
        fetch_smm_lme_inventories(client),
    )
...
async def get_commodities(refresh: bool = False, allow_stale: bool = True, force: bool = False) -> dict[str, Any]:
    ...
    async with httpx.AsyncClient(follow_redirects=True, timeout=httpx.Timeout(timeout)) as client:
        ...
        (
            ...
            (lme_inventories, lme_inventory_error),
        ) = await fetch_commodity_sources(client)
        inventories = merge_inventory_series(smm_inventories, shfe_inventories, eastmoney_inventories, lme_inventories)
    ...
    if lme_inventory_error:
        warnings.append(lme_inventory_error)
    ...
    data = {
        ...
        "source": "Sina Futures / Sina Spot / SMM / LME / SHFE / Sunsirs / Eastmoney",
...
def upgrade_commodity_snapshot(data: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    for item in snapshot.get("items") or []:
        ...
        for inventory_row in inventory_rows:
            market = infer_inventory_market(inventory_row)
            inventory_row["market"] = market
            inventory_row["marketLabel"] = INVENTORY_MARKET_LABELS[market]
        domestic_inventory = select_primary_inventory(item_id, inventory_rows, market="domestic")
        international_inventory = select_primary_inventory(item_id, inventory_rows, market="international")
        if domestic_inventory is None and item.get("inventory") is not None:
            domestic_inventory = {
                key: item.get(key)
                for key in [
                    "inventory",
            ...
            domestic_inventory.update({"market": "domestic", "marketLabel": INVENTORY_MARKET_LABELS["domestic"]})
        item["inventoryByMarket"] = {
            "domestic": domestic_inventory,
            "international": international_inventory,
        }
...
async def fetch_smm_lme_inventories(client: httpx.AsyncClient) -> tuple[dict[str, dict[str, Any]], str]:
    tasks = [
        client.get(
            SMM_LME_INVENTORY_API_URL.format(page=page),
            headers={
                "User-Agent": user_agent(),
                "Referer": SMM_LME_LIVE_URL,
                "Accept": "application/json,text/plain,*/*",
            },
        )
        for page in range(1, SMM_LME_HISTORY_PAGES + 1)
    ]
    results = await asyncio.gather(*tasks, return_exceptions=True)
    ...
    for page, result in enumerate(results, 1):
        if isinstance(result, Exception):
            errors.append(f"第{page}页 {type(result).__name__}: {result}")
            continue
        try:
            result.raise_for_status()
            payload = result.json()
            response_data = payload.get("data") if isinstance(payload, dict) else None
            code = (response_data.get("codemsg") or {}).get("code") if isinstance(response_data, dict) else None
            rows = ((response_data.get("data") or {}).get("listList") or []) if isinstance(response_data, dict) else []
            ...
            errors.append(f"第{page}页 {type(error).__name__}: {error}")

    parsed = parse_smm_lme_inventory_rows(source_rows)
    if parsed:
        warning = f"SMM LME库存部分历史页暂未取到（{'；'.join(errors[:2])}）" if errors else ""
        return parsed, warning
    detail = f"（{'；'.join(errors[:2])}）" if errors else ""
    return {}, f"SMM LME库存暂未取到{detail}"


def parse_smm_lme_inventory_rows(rows: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
    histories: dict[str, dict[str, dict[str, Any]]] = {commodity_id: {} for commodity_id in LME_INVENTORY_METALS}
    for row in rows:
        text = clean_html(" ".join(str(row.get(key) or "") for key in ["title", "profile", "content"]))
        if "LME" not in text or "库存" not in text:
            continue
        published_date = parse_smm_inventory_publish_date(row.get("date"))
        source_url = f"https://news.smm.cn/live/detail/{row.get('newsId')}" if row.get("newsId") else SMM_LME_LIVE_URL
        for commodity_id, metal_name in LME_INVENTORY_METALS.items():
            pattern = re.compile(
                rf"(?:【{re.escape(metal_name)}库存】)?(?:\d{{4}}年)?(?P<month>\d{{1,2}})月(?P<day>\d{{1,2}})日"
                rf"LME{re.escape(metal_name)}库存(?:(?P<flat>持平)|(?P<direction>增(?:加)?|减(?:少)?)(?P<change>[0-9,.]+))"
                rf"吨?至(?P<value>[0-9,.]+)吨"
            )
            ...
            if match.group("flat"):
                ...
                if change is not None and str(match.group("direction") or "").startswith("减"):
                    change = -change
            inventory_date = resolve_smm_inventory_date(
                published_date,
                int(match.group("month")),
                int(match.group("day")),
            )
            histories[commodity_id][inventory_date] = {
                "date": inventory_date,
                "value": value,
                "change": change,
    ...
    for commodity_id, points_by_date in histories.items():
        history = sorted(points_by_date.values(), key=lambda point: point["date"])[-HISTORY_POINTS:]
        if not history:
            continue
        latest = history[-1]
        change = safe_float(latest.get("change"))
        previous_value = latest["value"] - change if change is not None else (history[-2]["value"] if len(history) > 1 else None)
        change_pct = round(change / previous_value * 100, 2) if change is not None and previous_value else None
        parsed[commodity_id] = {
            ...
            "inventorySourceUrl": latest["sourceUrl"],
            "inventoryChange": change,
            "inventoryChangePct": change_pct,
            "inventoryHistory": [
                {key: point.get(key) for key in ["date", "value", "change"]}
                for point in history
            ],
            "inventoryType": "deliverable_stock",
...
def parse_smm_inventory_publish_date(value: Any) -> datetime:
    try:
        return datetime.fromisoformat(str(value or "")[:10]).replace(tzinfo=UTC)
    except ValueError:
        return datetime.now(UTC)


def resolve_smm_inventory_date(published_at: datetime, month: int, day: int) -> str:
    year = published_at.year
    candidate = datetime(year, month, day, tzinfo=UTC)
    if candidate > published_at + timedelta(days=31):
...
def infer_inventory_market(row: dict[str, Any]) -> str:
    declared = str(row.get("market") or row.get("inventoryMarket") or "").strip().lower()
    if declared in INVENTORY_MARKET_LABELS:
        return declared
    source = " ".join(
        str(row.get(key) or "")
        for key in ["inventorySource", "inventorySourceUrl", "exchange"]
    ).upper()
    if any(token in source for token in ["LME", "COMEX", "CME", "CBOT", "NYMEX", "ICE", "EIA"]):
    ...
    for source_rows in sources:
        for commodity_id, raw_row in source_rows.items():
            ...
            row["quality"] = build_metric_quality(
                ...
                definition=f"{INVENTORY_MARKET_LABELS[market]}{INVENTORY_TYPE_LABELS[inventory_type]}；不同市场或库存类型不可互相覆盖或直接求和",
                method="observed",
                status="ok" if row.get("inventory") is not None and inventory_date else "partial",
                coverage={"market": market, "exchange": str(row.get("exchange") or ""), "inventoryType": inventory_type},
...
def select_primary_inventory(
    commodity_id: str,
    rows: list[dict[str, Any]],
    *,
    market: str = "domestic",
) -> dict[str, Any] | None:
    market_rows = [row for row in rows if infer_inventory_market(row) == market]
    if not market_rows:
        return None
    preferred_type = PREFERRED_INVENTORY_TYPES.get(commodity_id, "exchange_receipt") if market == "domestic" else "deliverable_stock"
    preferred = [row for row in market_rows if row.get("inventoryType") == preferred_type]
    candidates = preferred or market_rows
...
def build_commodity_item(
    ...
    domestic_inventory = select_primary_inventory(definition["id"], inventory_rows, market="domestic")
    international_inventory = select_primary_inventory(definition["id"], inventory_rows, market="international")
    ...
    return {
        ...
        **catalog_metadata,
        ...
        "inventory": domestic_inventory.get("inventory") if domestic_inventory else None,
        "inventoryUnit": domestic_inventory.get("inventoryUnit") if domestic_inventory else "",
        "inventoryDate": domestic_inventory.get("inventoryDate") if domestic_inventory else "",
        "inventorySource": domestic_inventory.get("inventorySource") if domestic_inventory else "",
        "inventorySourceUrl": inventory_source_url(domestic_inventory) if domestic_inventory else "",
        "inventoryChange": domestic_inventory.get("inventoryChange") if domestic_inventory else None,
        "inventoryChangePct": domestic_inventory.get("inventoryChangePct") if domestic_inventory else None,
        "inventoryHistory": domestic_inventory.get("inventoryHistory", []) if domestic_inventory else [],
        "inventoryType": domestic_inventory.get("inventoryType") if domestic_inventory else "",
        "inventoryTypeLabel": domestic_inventory.get("inventoryTypeLabel") if domestic_inventory else "",
        "inventoryByMarket": {
            "domestic": domestic_inventory,
            "international": international_inventory,
        },
            ...
            sorted(
                {
                    ...
                    for value in [
                        ...
                        domestic_inventory.get("inventorySource") if domestic_inventory else "",
                        international_inventory.get("inventorySource") if international_inventory else "",
                    ]
                    if value
                }
            )
