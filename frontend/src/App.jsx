...
import { compactIssueSummary, joinIssueDetails } from "./status-utils.js";
import { listCommodityInventories, selectCommodityInventory } from "./commodity-market.js";
import { loadStockPageSources, stockSectionViewState } from "./stock-page-state.js";
...
const PAGE_REFRESH_KINDS = {
  news: ["news"],
  ai: ["ai-news", "ai-projects"],
  xueqiu: ["xueqiu"],
  stocks: ["stocks"],
  ...
  macro: ["macro"],
  games: ["games", "games-region"]
};
const REFRESH_KIND_LABELS = {
  news: "资讯",
  "ai-news": "AI 新闻",
  "ai-projects": "AI 工具",
  xueqiu: "雪球",
...
const TODAY_DATA_PAGES = PAGE_GROUPS
  .flatMap((group) => group.pages)
  .filter(({ page }) => PAGE_REFRESH_KINDS[page])
  .map(({ page, href, label }) => ({
    id: page,
    healthId: page === "ai" ? "ai_news" : page,
    href,
    label,
    refreshKinds: PAGE_REFRESH_KINDS[page]
  }));
...
const STOCK_SECTION_LINKS = [
  { id: "stock-market", label: "市场概览", detail: "全球流动性", icon: LineChart },
  { id: "stock-signals", label: "边际信号", detail: "交易日雷达", icon: TrendingUp },
  { id: "stock-industry", label: "机构与融资", detail: "申万行业全景", icon: Landmark }
];
...
function refreshTaskNames(entries) {
  return entries.map(({ kind }) => REFRESH_KIND_LABELS[kind] || kind).join("、");
}

function buildTodayHealthCards(healthItems, refreshStatus) {
  const healthById = new Map((healthItems || []).map((item) => [item.id, item]));

  return TODAY_DATA_PAGES.map((page) => {
    const health = healthById.get(page.healthId);
    const taskStates = page.refreshKinds
      .map((kind) => ({ kind, state: refreshStatus?.[kind] }))
      .filter(({ state }) => state);
    const active = taskStates.filter(({ state }) => state.status === "running" && state.phase !== "queued");
    const queued = taskStates.filter(({ state }) => state.status === "running" && state.phase === "queued");
    const failed = taskStates.filter(({ state }) => state.status === "error");
    const refreshAsOf = taskStates
      .map(({ state }) => state.finishedAt || state.startedAt || "")
    ...
    const base = {
      ...
      coverage: health?.coverage || `${page.refreshKinds.length} 项启动任务`,
      asOf: refreshAsOf || health?.asOf || ""
    };

    if (active.length) {
      const notes = [`${refreshTaskNames(active)}正在刷新`];
      if (queued.length) notes.push(`${refreshTaskNames(queued)}排队中`);
      if (failed.length) notes.push(`${refreshTaskNames(failed)}刷新失败`);
      return { ...base, status: "running", activity: "running", statusLabel: "刷新中", note: notes.join("；") };
    }

    if (queued.length) {
      const notes = [`${refreshTaskNames(queued)}等待刷新`];
      if (failed.length) notes.push(`${refreshTaskNames(failed)}刷新失败`);
      return { ...base, status: "queued", activity: "queued", statusLabel: "排队中", note: notes.join("；") };
    }

    if (failed.length) {
      const messages = [...new Set(failed.map(({ state }) => state.message).filter(Boolean))];
      return {
        ...base,
        status: "error",
        activity: "error",
        statusLabel: "刷新失败",
        note: messages.join("；") || `${refreshTaskNames(failed)}刷新失败`
      };
    }
    ...
    if (health) {
      return {
        ...
        activity: "finished",
        statusLabel: TODAY_HEALTH_LABELS[health.status] || health.status,
        note: health.note,
        coverage: health.coverage || base.coverage,
        asOf: health.asOf || refreshAsOf
      };
    }

    if (taskStates.some(({ state }) => state.status === "done")) {
      return { ...base, status: "ok", activity: "finished", statusLabel: "刷新完成", note: "启动刷新已完成" };
    }

    if (taskStates.some(({ state }) => state.status === "skipped")) {
      return { ...base, status: "stale", activity: "finished", statusLabel: "已有快照", note: "本轮沿用最近有效快照" };
    }
    ...
    return {
      ...
      status: "unavailable",
      activity: "idle",
      statusLabel: taskStates.length ? "等待启动" : "读取中",
      note: taskStates.length ? "等待服务启动刷新" : "正在读取启动刷新状态"
    };
  });
}
...
function TodayPage() {
  ...
  const loadRefreshStatus = useCallback(async () => {
    try {
      setRefreshStatus(await getJson(`/api/refresh-status?t=${Date.now()}`));
    } catch {
      // Keep the last visible refresh state when a status poll is transiently unavailable.
    }
  }, []);

  useEffect(() => {
    loadRefreshStatus();
    const timer = window.setInterval(loadRefreshStatus, STATUS_POLL_MS);
    return () => window.clearInterval(timer);
  }, [loadRefreshStatus]);

  const todayHealthCards = useMemo(
    () => buildTodayHealthCards(dashboard.health, refreshStatus),
    [dashboard.health, refreshStatus]
  );
  const activePageLabels = todayHealthCards.filter((item) => item.activity === "running").map((item) => item.label);
  const queuedPageLabels = todayHealthCards.filter((item) => item.activity === "queued").map((item) => item.label);
  const failedPageLabels = todayHealthCards.filter((item) => item.hasFailure).map((item) => item.label);
  const refreshStatusReady = Object.keys(refreshStatus).length > 0;
  const healthSectionParts = [];
  if (activePageLabels.length) healthSectionParts.push(`正在刷新：${activePageLabels.join("、")}`);
  if (queuedPageLabels.length) healthSectionParts.push(`排队中：${queuedPageLabels.join("、")}`);
  if (failedPageLabels.length) healthSectionParts.push(`刷新失败：${failedPageLabels.join("、")}`);
  if (!activePageLabels.length && !queuedPageLabels.length) {
    healthSectionParts.push(
      refreshStatusReady
        ? failedPageLabels.length ? "其余任务已结束" : "本轮刷新已完成"
        : "正在读取刷新状态"
    );
  }
  const healthSectionNote = healthSectionParts.join("；");
  const healthSectionTone = activePageLabels.length
    ? "running"
    : queuedPageLabels.length
      ? "queued"
      : failedPageLabels.length
        ? "error"
        : "done";
    ...
    >
      ...
      <section className="today-section" aria-labelledby="today-health-title">
        <TodaySectionTitle
          ...
          live
          note={healthSectionNote}
          noteTone={healthSectionTone}
          title="数据健康"
        />
        <div className="today-health-grid">
          {todayHealthCards.map((item) => (
            <a
              aria-busy={item.activity === "running"}
              className={`today-health-card is-${item.status || "unavailable"}`}
              href={item.href}
              key={item.id}
            >
              <span>{item.label}</span>
              <strong>
                {item.activity === "running" ? <RefreshCw className="today-health-refresh-icon" size={11} aria-hidden="true" /> : null}
                {item.statusLabel}
              </strong>
          ...
          <div className="today-change-grid">
            {dashboard.changes.map((item) => (
              <article className="today-change-card" key={item.id}>
                ...
                <footer>
                  <time dateTime={item.quality?.asOf} title={item.quality?.asOf}>{formatTodayCardDate(item.quality?.asOf)}</time>
...
function TodaySectionTitle({ icon, id, title, note, noteTone = "", live = false }) {
  return (
    <header className="today-section-title">
      <div>{icon}<h2 id={id}>{title}</h2></div>
      <span aria-live={live ? "polite" : undefined} className={noteTone ? `is-${noteTone}` : undefined}>{note}</span>
    </header>
  );
}
...
function formatTodayCardDate(value) {
  if (!value) return "时间不可见";
...
function AiPage() {
  ...
  const followedProjectsKey = (aiPreferences.followedProjects || []).join("\u0000");
  const ignoredProjectsKey = (aiPreferences.ignoredProjects || []).join("\u0000");
  const alertSignals = useMemo(
    () => selectAiAlertSignals(projectsData.projects, projectsData.signals, aiPreferences),
    [projectsData.projects, projectsData.signals, followedProjectsKey, ignoredProjectsKey]
  );
  ...
  const enableAiNotifications = useCallback(async () => {
    ...
    const unseen = alertSignals.filter((signal) => !next.seenSignalIds?.includes(signal.eventId));
    const notifiedSignalIds = notifyAiSignals(unseen, next);
    next = markAiSignalsSeen(next, notifiedSignalIds);
    setAiPreferences(next);
    saveAiPreferences(next);
    setNotificationStatus(notifiedSignalIds.length ? `已开启系统通知，并发送 1 条汇总通知（包含 ${notifiedSignalIds.length} 条新提醒）。` : "已开启系统通知；有新的高信号时会提醒。 ");
  }, [aiPreferences, alertSignals]);

  useEffect(() => {
    if (!aiPreferences.systemNotifications || globalThis.Notification?.permission !== "granted") return;
    const unseen = alertSignals.filter((signal) => !aiPreferences.seenSignalIds?.includes(signal.eventId));
    ...
    setAiPreferences(next);
    saveAiPreferences(next);
  }, [projectsData.generatedAt, aiPreferences.systemNotifications, alertSignals]);
...
function AiProjectRanking({
  ...
  const highSignals = alertSignals || [];
  const digestSignals = data.signals?.digest || [];
  const followedReleaseCount = highSignals.filter((signal) => signal.followedRelease).length;
  const signalDisplay = buildAiSignalDisplay(highSignals, signalsExpanded);
  const signalSummaryParts = [];
  if (signalDisplay.hiddenCount) signalSummaryParts.push(`当前显示 ${signalDisplay.items.length}/${signalDisplay.totalCount} 条`);
  else if (signalsExpanded && signalDisplay.totalCount > 4) signalSummaryParts.push(`已显示全部 ${signalDisplay.totalCount} 条`);
  if (followedReleaseCount) signalSummaryParts.push(`其中 ${followedReleaseCount} 条关注库版本更新`);
  else signalSummaryParts.push(`${digestSignals.length} 条变化已进入摘要`);
  ...
  return (
    <section className="ai-projects-panel ai-discovery" role="tabpanel" aria-live="polite">
      <div className="ai-discovery-hero">
        <div>
          <span className="github-category-kicker">按 Stars 总数与真实增长发现 AI 项目</span>
          <h2>AI 工具雷达</h2>
          <p>{activeView.description}</p>
        </div>
        <div className="ai-discovery-hero-meta">
          <strong>{data.summary?.defaultVisibleCount || 0}</strong>
          <span>个 Stars 达标工具</span>
          <small>总 Stars ≥ 1,000，或 7 天增长 ≥ 100，或 30 天增长 ≥ 500；增长采集中不入榜。</small>
        </div>
      </div>
      ...
      <aside className="ai-signal-summary" aria-label="AI 项目提醒摘要">
          <div className="ai-signal-summary-title">
            <div>
              ...
              <span>{signalSummaryParts.join(" · ")}</span>
            </div>
          ...
          {!!highSignals.length && (
            <div className="ai-signal-list" id="ai-signal-list">
              {signalDisplay.items.map((signal) => (
                <a key={signal.eventId} href={signal.url || "#"} target={signal.url ? "_blank" : undefined} rel={signal.url ? "noreferrer" : undefined}>
                  <Zap size={14} aria-hidden="true" />
                  <span><strong>{signal.title}</strong><small>{signal.followedRelease ? `关注库新版 · ${signal.reason}` : signal.reason}</small></span>
                </a>
              ))}
            </div>
          )}
          {highSignals.length > 4 && (
            <button
              aria-controls="ai-signal-list"
              aria-expanded={signalsExpanded}
              className="ai-signal-toggle"
              type="button"
              onClick={() => setSignalsExpanded((current) => !current)}
            >
              {signalsExpanded ? "收起提醒" : `查看全部 ${signalDisplay.totalCount} 条（还有 ${signalDisplay.hiddenCount} 条）`}
            </button>
          )}
      ...
      <div className="section-title compact ai-discovery-result-title">
        ...
        <small>{activeView.id === "rising" ? "按 7 天、30 天 Stars 增长数排序" : "按 Stars 总数排序；增长数仅作同 Stars 次序"}</small>
      </div>
            ...
            return (
              <article className="ai-project-card" key={project.id || project.fullName}>
                ...
                <div className="ai-project-metrics">
                  <div><small>入榜依据</small><strong>{getAiProjectSelectionLabel(project)}</strong></div>
...
function StocksPage({ stockId = "" }) {
  ...
  const [activeStockSection, setActiveStockSection] = useState(STOCK_SECTION_LINKS[0].id);

  const knownMarketSignatures = useRef(new Map());
  const lastStatusText = useRef("");
  const lastRefreshFinishedAt = useRef("");
  const hasResolvedStockSnapshot = useRef(false);
  const sectionJumpNavRef = useRef(null);

  const loadStocks = useCallback(async ({ markNew = false } = {}) => {
    if (!hasResolvedStockSnapshot.current) {
      setStocksLoading(true);
      setStatus("正在读取最近股票快照...");
    }

    const result = await loadStockPageSources({
      fetchStocks: () => getJson(`/api/stocks?t=${Date.now()}`),
      fetchWatchlist: () => getJson(`/api/stock-watchlist?t=${Date.now()}`),
      onStocks: (data) => {
        ...
        hasResolvedStockSnapshot.current = true;
        setStocksLoading(false);

        const statusText = buildStocksStatus(data);
        lastStatusText.current = statusText;
        setStatus(statusText);
      },
      onStocksError: (error) => {
        setStocksLoading(false);
        const statusText = `股票快照读取失败：${error.message}`;
        lastStatusText.current = statusText;
        setStatus(statusText);
      },
      onWatchlist: (watchData) => {
        ...
        const statusText = withWatchlistCount(lastStatusText.current || "股票快照状态待更新", nextWatchlist.length);
        lastStatusText.current = statusText;
        setStatus(statusText);
      },
      onWatchlistError: (error) => {
        setWatchlistLoading(false);
        const statusText = `${lastStatusText.current || "股票快照状态待更新"}；自选读取失败：${error.message}`;
        lastStatusText.current = statusText;
        setStatus(statusText);
      }
    });

    return result.stocks.ok;
  }, [stockId]);
  ...
  useEffect(() => {
    if (stockTab !== "market") return undefined;
    const nav = sectionJumpNavRef.current;
    const panel = nav?.parentElement;
    const topbar = nav?.closest("main")?.querySelector(".topbar");
    const sections = STOCK_SECTION_LINKS
      .map(({ id }) => document.getElementById(id))
      .filter(Boolean);
    if (!nav || !panel || !topbar || !sections.length) return undefined;

    let frameId = 0;
    let stickyTop = 8;

    const updateActiveSection = () => {
      const triggerY = stickyTop + nav.getBoundingClientRect().height + 24;
      let nextActive = sections[0].id;
      sections.forEach((section) => {
        if (section.getBoundingClientRect().top <= triggerY) nextActive = section.id;
      });
      setActiveStockSection((current) => (current === nextActive ? current : nextActive));
    };

    const updateStickyOffset = () => {
      const mobile = window.matchMedia("(max-width: 900px)").matches;
      stickyTop = mobile ? 8 : Math.ceil(topbar.getBoundingClientRect().bottom + 8);
      nav.style.setProperty("--stock-section-nav-top", `${stickyTop}px`);
      panel.style.setProperty(
        "--stock-section-anchor-offset",
        `${stickyTop + nav.getBoundingClientRect().height + 16}px`
      );
      updateActiveSection();
    };
  ...
  const marketViewState = stockSectionViewState({
    loading: stocksLoading,
    hasData: markets.length > 0
  });
  const signalsViewState = stockSectionViewState({
    loading: stocksLoading,
    hasData: Boolean(marginalSignals?.cards?.length)
  });
  const industryViewState = stockSectionViewState({
    loading: stocksLoading,
    hasData: Boolean(
      institutionAllocation?.categories?.length
      || industryFinancingTrend?.dates?.length
    )
  });
    ...
    >
      <div className="stock-subtabs" role="tablist" aria-label="股票功能">
        ...
        >
          <strong>
            个股{" "}
            <em aria-label={watchlistLoading ? "自选股票正在读取" : `${watchlist.length}只自选股票`}>
      ...
      {stockTab === "market" ? (
        <div id="stock-panel-market" className="stock-tab-panel" role="tabpanel" aria-labelledby="stock-tab-market">
          <nav ref={sectionJumpNavRef} className="stock-section-nav" aria-label="大盘页面分区">
            <div className="stock-section-nav-intro" aria-hidden="true">
              <span><Compass size={17} strokeWidth={2.2} /></span>
              <div>
                <strong>大盘导览</strong>
                <small>三层信号 · 滚动跟随</small>
              </div>
            </div>
            <div className="stock-section-nav-links">
              {STOCK_SECTION_LINKS.map(({ id, label, detail, icon: Icon }, index) => (
                <a
                  key={id}
                  className={activeStockSection === id ? "is-active" : ""}
                  href={`#${id}`}
                  aria-current={activeStockSection === id ? "location" : undefined}
                  onClick={() => setActiveStockSection(id)}
                >
                  <span className="stock-section-nav-icon"><Icon size={17} strokeWidth={2.2} /></span>
                  <span className="stock-section-nav-copy">
                    <strong>{label}</strong>
                    <small>{detail}</small>
                  </span>
                  <span className="stock-section-nav-index" aria-hidden="true">0{index + 1}</span>
                </a>
              ))}
            </div>
          </nav>
          <section id="stock-market" className="dashboard-section" aria-labelledby="stock-market-title">
            ...
            <div className="market-grid" aria-live="polite" aria-busy={marketViewState === "loading"}>
              {marketViewState === "loading" ? (
                <StockSnapshotLoading
                  label="正在读取全球市场快照"
                  detail="命中本地快照后立即展示，不等待自选股数据。"
                  cards={3}
                />
              ) : marketViewState === "empty" ? (
          ...
          <div id="stock-signals" className="dashboard-section-anchor">
            {signalsViewState === "loading" ? (
              <StockSnapshotLoading
                label="正在读取边际信号快照"
                detail="资金、对冲、供给与市场宽度会从最近可用版本恢复。"
                cards={4}
              />
            ) : (
          ...
          <div id="stock-industry" className="dashboard-section-anchor">
            {industryViewState === "loading" ? (
              <StockSnapshotLoading
                label="正在读取机构与融资快照"
                detail="机构持仓与近三年融资序列正在从本地快照恢复。"
                cards={3}
                tall
              />
            ) : (
...
function StockSnapshotLoading({ label, detail, cards = 3, tall = false }) {
  return (
    <div className={`stock-snapshot-loading${tall ? " is-tall" : ""}`} role="status">
      <div className="stock-snapshot-loading-copy">
        <span><Activity size={18} strokeWidth={2.25} /></span>
        <div>
          <strong>{label}</strong>
          <small>{detail}</small>
        </div>
      </div>
      <div className="stock-snapshot-loading-grid" aria-hidden="true">
        {Array.from({ length: cards }, (_, index) => (
          <div key={index} className="stock-snapshot-loading-card">
            <span />
            <span />
            <span />
            <span />
        ...
        {marketRows.map((row) => (
          <div key={`${item.id}-${row.id}`} className={`commodity-product-grid commodity-market-line commodity-market-line-${row.id}`}>
            ...
            <div className="commodity-inventory-cell" data-label="库存">
              <CommodityInventoryCell itemName={item.name} row={row} />
            </div>
...
function buildCommodityMarketRows(item) {
  const rows = [];
  const domesticInventory = selectCommodityInventory(item, "domestic");
  const internationalInventory = selectCommodityInventory(item, "international");
  const hasDomestic = hasValue(item.domesticFuturePrice) || hasValue(item.spotPrice) || hasValue(item.basis) || Boolean(domesticInventory);
  const hasGlobal = hasValue(item.globalFuturePrice) || hasValue(item.benchmarkFuturePrice) || (item.globalFutureHistory || []).length > 0 || (item.benchmarkFutureHistory || []).length > 0 || Boolean(internationalInventory);
  ...
  if (hasDomestic) {
    rows.push({
      ...
      inventory: domesticInventory,
      inventorySeries: listCommodityInventories(item, "domestic"),
  ...
  if (hasGlobal) {
    ...
    rows.push({
      ...
      inventory: internationalInventory,
      inventorySeries: listCommodityInventories(item, "international"),
      sourceText: [useGlobal ? "新浪外盘" : relationLabel, internationalInventory?.inventorySource, item.note].filter(Boolean).join("；")
    });
  }
...
function CommodityInventoryCell({ itemName, row }) {
  const inventory = row.inventory;
  if (!hasValue(inventory?.inventory)) {
    return <span className="sparkline-empty">{row.id === "international" ? "暂无对应海外库存" : "暂无国内库存"}</span>;
  }
  return (
    <>
      {formatInventory(inventory)}
      {inventory.inventoryTypeLabel && <small>{inventory.inventoryTypeLabel}</small>}
      {hasValue(inventory.inventoryChange) && <small className={pctClass(inventory.inventoryChange)}>{formatInventoryChange(inventory)}</small>}
      <MiniKline history={inventory.inventoryHistory} label={`${itemName}${row.label}库存K线`} />
      {inventory.inventoryDate && <small>{inventory.inventoryDate}</small>}
      {(row.inventorySeries || []).length > 1 && <small>另有 {(row.inventorySeries || []).length - 1} 类库存，未合并</small>}
    </>
  );
}
...
function CommoditySourceCell({ item, row }) {
  ...
  return (
    <div className="commodity-source-cell" title={sourceText} data-label="来源">
      <strong>{row.date || row.inventory?.inventoryDate || ""}</strong>
...
function compactCommoditySource(value, marketId = "") {
  ...
  labels.forEach(([needle, label]) => {
    ...
    if (marketId === "domestic" && label === "LME库存") return;
...
function GameTop100TableV2({ market, newKeys, showRankings = true }) {
  ...
  const coverage = market?.coverage || {
    targetCount: market?.rankLimit || 100,
    availableCount: rows.length,
    missingCount: Math.max((market?.rankLimit || 100) - rows.length, 0),
  };
  ...
  return (
    <section className="game-section game-top100">
      ...
      <div className={`game-market-coverage ${coverage.missingCount || coverage.status === "partial" ? "is-partial" : "is-complete"}`}>
        <p><strong>统计口径：</strong>{coverage.status === "partial" ? "部分覆盖；" : ""}{formatMarketCoverageV2(coverage)}</p>
        <p>
          <strong>数据缺口：</strong>目标 {coverage.targetCount || market?.rankLimit || 100} 条，
          可用 {coverage.availableCount ?? rows.length} 条，缺少 {coverage.missingCount ?? 0} 条；缺口不补 0、不伪造排名。
        </p>
        {coverage.status === "partial" && coverage.missingReason && <small>{coverage.missingReason}</small>}
      </div>
      <div className="stock-table-wrap">
            ...
            {!rows.length ? (
              ...
              rows.map((row) => (
                <tr key={gameMarketRowKeyV2(market, row)} className={newKeys.has(gameMarketRowKeyV2(market, row)) ? "is-new-row" : ""}>
                  ...
                  <td>
                    <strong>{formatRevenueExact(row.revenue)}</strong>
                    <small>{formatRevenueSourceV2(row)}</small>
                    {row.coverageNote && <small className="game-revenue-coverage">口径：{row.coverageNote}</small>}
...
function buildNewsStatus(data, news) {
  ...
  const errors = (data.errors || []).map((item) => String(item || "").trim()).filter(Boolean);
  const warnings = (data.warnings || []).map((item) => String(item || "").trim()).filter(Boolean);
  const warningHint = warnings.length ? `；来源提示：${compactIssueSummary(warnings)}` : "";
  const warningDetails = warnings.length > 1 ? `；来源提示明细：${joinIssueDetails(warnings)}` : "";
  const errorDetails = errors.length > 1 ? `；来源明细：${errors.join("；")}` : "";
  const errorHint = errors.length ? `；来源异常：${compactIssueSummary(errors)}` : "";
  return `已更新：${formatTime(data.generatedAt)}${cacheHint}${storageHint}${throttleHint}${staleHint}${countHint}；来源：${sources}${warningHint}${warningDetails}${errorDetails}${errorHint}`;
}
...
function buildGamesStatusV2(data) {
  const summary = data.summary || {};
  const usableStatuses = new Set(["imported", "authorized_api", "public_fallback", "credentials_present"]);
...
function providerStatusLabelV2(status) {
  ...
  if (status === "authorized_api") return "授权 API 已接入";
  if (status === "public_fallback") return "公开兜底";
  if (status === "configuration_required") return "授权需检查 / 公开兜底中";
...
function formatMarketCoverageV2(coverage = {}) {
  const methodLabels = {
    estimated: "模型估算",
    reported: "披露数据",
    observed: "官方数据",
  ...
  const parts = [];
  if (coverage.period) parts.push(formatMonth(coverage.period));
  if (coverage.methods?.length) parts.push(coverage.methods.map((item) => methodLabels[item] || item).join(" / "));
  if (coverage.coveredRegions?.length) parts.push(`地区 ${coverage.coveredRegions.join("、")}`);
  if (coverage.platforms?.length) parts.push(`平台 ${coverage.platforms.join("、")}`);
  if (coverage.excluded?.length) parts.push(`未统计 ${coverage.excluded.join("、")}`);
  return parts.join("；") || "来源未声明完整覆盖范围，未覆盖地区保持缺失";
}
...
function buildGenericStatus(data, source) {
  ...
  const warningHint = data.warnings?.length ? `；口径/授权提示：${data.warnings.join("；")}` : "";
  return `已更新：${formatTime(data.generatedAt)}${cacheHint}${storageHint}${throttleHint}${staleHint}；来源：${source}${warningHint}${errorHint}`;
}
...
function formatRevenueExact(value) {
  if (!value) return "待导入";
  const amount = typeof value === "object" ? value.amount : value;
  const currency = (typeof value === "object" ? value.currency : "USD") || "USD";
  if (amount === null || amount === undefined || Number.isNaN(Number(amount))) {
    return typeof value === "object" ? value.raw || "待导入" : "待导入";
  }
  try {
    ...
    return `${Number(amount).toLocaleString("zh-CN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ${currency}`;
  }
}
