...
def test_v93_v128_game_dashboard_preserves_sensor_top100_but_filters_watchlist_rankings():
    revenue_rows = [
    ...
    assert [row["rank"] for row in filtered_revenue] == [50, 1, 51]
    assert filtered_revenue[0]["watchlistId"] == "heartopia"
    assert "watchlistId" not in filtered_revenue[1]
    assert filtered_revenue[2]["watchlistId"] == "heartopia"
    assert [(row["rank"], row["watchlistId"]) for row in filtered_rankings] == [
        (9, "heartopia"),
        (42, "heartopia"),
    ]
    assert not any("Sensor Tower：排除" in warning for warning in warnings)
    assert any("七麦：排除 1 条" in warning for warning in warnings)


def test_v92_v95_game_payload_empty_watchlist_keeps_sensor_top100_but_filters_rankings():
    ...
    global_market = next(market for market in result["markets"] if market["id"] == "global")
    assert [row["game"] for row in global_market["top100"]] == ["Old Game"]
    assert all(
        chart["rows"] == []
