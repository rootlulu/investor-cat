...
class StockRefreshConcurrencyTests(unittest.IsolatedAsyncioTestCase):
    async def test_source_capture_does_not_swallow_cancellation(self) -> None:
        async def cancelled() -> None:
            raise asyncio.CancelledError

        with self.assertRaises(asyncio.CancelledError):
            await capture_stock_source(cancelled())

    async def test_independent_sources_overlap_and_marginal_waits_only_for_markets(self) -> None:
        async_started: set[str] = set()
        async_gate = asyncio.Event()
        sync_started: set[str] = set()
        sync_lock = threading.Lock()
        sync_gate = threading.Event()
        release_siblings = threading.Event()

        async def async_source(name: str) -> dict:
            async_started.add(name)
            if len(async_started) == 2:
                async_gate.set()
            await asyncio.wait_for(async_gate.wait(), timeout=0.25)
            return {"industries": [{"code": name, "name": name}]}

        async def china_source(*_args) -> dict:
            return await async_source("china")

        async def world_source(*_args) -> dict:
            return await async_source("world")

        def mark_sync_started(name: str) -> None:
            with sync_lock:
                sync_started.add(name)
                if len(sync_started) == 3:
                    sync_gate.set()
            if not sync_gate.wait(0.5):
                raise RuntimeError("independent sync sources did not overlap")

        def market_source(_db_path: Path, _timeout: float) -> list[dict]:
            mark_sync_started("markets")
            return [{"id": "a_share", "status": "ok", "marketCap": 100.0, "indices": [{"symbol": "000001"}]}]

        def industry_source(_timeout: float, _previous: dict | None) -> dict:
            mark_sync_started("industry")
            if not release_siblings.wait(0.5):
                raise RuntimeError("marginal signals waited for unrelated industry source")
            return {"status": "ok", "dates": ["2026-07-28"], "series": []}

        def institution_source(_timeout: float) -> dict:
            mark_sync_started("institution")
            if not release_siblings.wait(0.5):
                raise RuntimeError("marginal signals waited for unrelated institution source")
            return {"status": "ok", "categories": [{"id": "public_fund"}], "errors": []}

        def marginal_source(markets: list[dict], _timeout: float) -> dict:
            self.assertTrue(sync_gate.is_set())
            self.assertEqual(markets[0]["id"], "a_share")
            release_siblings.set()
            return {"cards": []}

        with (
            patch("src.stock_service.load_config", return_value={"fetch": {"min_refresh_interval_seconds": 1800}}),
            patch("src.stock_service.resolve_sqlite_path", return_value=Path("test.sqlite")),
            patch("src.stock_service.load_latest_stocks", new=AsyncMock(return_value=None)),
            patch("src.stock_service.save_latest_stocks", new=AsyncMock(return_value=None)),
            patch("src.stock_service.fetch_china_market", side_effect=china_source),
            patch("src.stock_service.fetch_world_market", side_effect=world_source),
            patch("src.stock_service.fetch_market_overviews_sync", side_effect=market_source),
            patch("src.stock_service.fetch_industry_financing_trend_sync", side_effect=industry_source),
            patch("src.stock_service.fetch_institution_industry_allocation_sync", side_effect=institution_source),
            patch("src.stock_service.fetch_marginal_signals_sync", side_effect=marginal_source),
        ):
            result = await get_stocks(refresh=True, allow_stale=False, force=True)

        self.assertEqual(result["errors"], [])
        self.assertEqual(async_started, {"china", "world"})
        self.assertEqual(sync_started, {"markets", "industry", "institution"})
        self.assertTrue(release_siblings.is_set())

    async def test_one_source_failure_does_not_cancel_successful_siblings(self) -> None:
        async def failed_china(*_args) -> dict:
            raise RuntimeError("china down")

        async def world(*_args) -> dict:
            await asyncio.sleep(0)
            return {"industries": [{"code": "world"}]}

        def failed_markets(*_args) -> list[dict]:
        ...
        with (
            ...
            patch("src.stock_service.load_latest_stocks", new=AsyncMock(return_value=None)),
            patch("src.stock_service.save_latest_stocks", new=AsyncMock(return_value=None)),
            patch("src.stock_service.fetch_china_market", side_effect=failed_china),
            patch("src.stock_service.fetch_world_market", side_effect=world),
            patch("src.stock_service.fetch_market_overviews_sync", side_effect=failed_markets),
            patch(
                "src.stock_service.fetch_industry_financing_trend_sync",
                return_value={"status": "ok", "dates": ["2026-07-28"], "series": []},
            ),
            patch(
                "src.stock_service.fetch_institution_industry_allocation_sync",
                return_value={"status": "ok", "categories": [{"id": "qfii"}], "errors": []},
            ),
            patch(
                "src.stock_service.fetch_marginal_signals_sync",
                return_value={"cards": [{"id": "funding", "status": "ok", "metrics": [{"value": 1}]}]},
            ),
        ):
            result = await get_stocks(refresh=True, allow_stale=False, force=True)

        self.assertEqual(result["world"]["industries"][0]["code"], "world")
        self.assertEqual(result["industryFinancingTrend"]["dates"], ["2026-07-28"])
        self.assertEqual(result["institutionIndustryAllocation"]["categories"][0]["id"], "qfii")
        self.assertEqual(result["marginalSignals"]["cards"][0]["id"], "funding")
        self.assertIn("中国股票数据：china down", result["errors"])
        self.assertIn("市场流动性数据：markets down", result["errors"])
