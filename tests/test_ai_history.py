...
class AiProjectHistoryTests(unittest.TestCase):
    ...
    def test_enrichment_budget_rotates_to_unchecked_and_oldest_projects(self) -> None:
        projects = [
            {
                "id": index,
                "fullName": f"popular/tool-{index}",
                "defaultVisible": True,
                "discoveryScore": index,
                "stars": 100_000 - index,
                "enrichmentCheckedAt": "2026-07-28T08:00:00+00:00",
            }
        ...
        projects.extend(
            [
                {
                    "id": 100,
                    "fullName": "framework/never-checked",
                ...
                {
                    "id": 101,
                    "fullName": "framework/oldest-check",
                    "defaultVisible": False,
                    "discoveryScore": 2,
                    "stars": 20,
                    "enrichmentCheckedAt": "2026-06-01T00:00:00+00:00",
                },
            ]
        )

        selected = select_enrichment_candidates(projects, authenticated=False)
        selected_names = [project["fullName"] for project in selected]

        self.assertEqual(len(selected), 12)
        self.assertIn("framework/never-checked", selected_names)
        self.assertIn("framework/oldest-check", selected_names)
        self.assertLess(
            selected_names.index("framework/never-checked"),
            selected_names.index("popular/tool-0"),
        )
        self.assertIn("popular/tool-0", selected_names)
        self.assertNotIn("popular/tool-11", selected_names)
    ...
    def test_high_signals_require_usable_stage_and_stable_event_ids(self) -> None:
        ...
        release_signal = next(signal for signal in first["high"] if signal["type"] == "release")
        self.assertEqual(release_signal["projectKey"], "demo/useful")
        self.assertEqual(release_signal["fullName"], "demo/useful")
        next_release = build_project_signals(
            {
                **usable,
                "release": {**usable["release"], "tag": "v1.3.0"},
            },
            now=now,
        )
        next_release_signal = next(signal for signal in next_release["high"] if signal["type"] == "release")
        self.assertNotEqual(release_signal["eventId"], next_release_signal["eventId"])
        self.assertFalse(hidden["high"])
        self.assertTrue(hidden["digest"])

        prerelease = build_project_signals(
            {
                **usable,
                "release": {**usable["release"], "tag": "v2.0.0-rc1", "prerelease": True},
            },
            now=now,
        )
        self.assertFalse(any(signal["type"] == "release" for signal in prerelease["digest"]))
...
class AiProjectEnrichmentTests(unittest.IsolatedAsyncioTestCase):
    async def test_enrichment_failure_keeps_last_known_good_metadata(self) -> None:
        ...
        self.assertTrue(enrichment["enrichmentCheckedAt"])
