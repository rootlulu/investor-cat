...
class StartupRefreshTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_startup_status_distinguishes_queued_from_active_work(self) -> None:
        state = REFRESH_STATE["news"]
        original = dict(state)
        ...
        async def execute(*_args, **_kwargs) -> dict:
        ...
        try:
            with patch("src.background_refresh.run_refresh", new_callable=AsyncMock) as scheduled_refresh:
                queued = await start_background_refresh("news", "startup", force=True)
                await asyncio.sleep(0)

            self.assertEqual(queued["phase"], "queued")
            scheduled_refresh.assert_awaited_once()

            run_id = int(state["runId"])
            with (
                patch("src.background_refresh.startup_refresh_slot", open_slot),
                patch("src.background_refresh.execute_refresh", side_effect=execute),
            ):
                await run_refresh("news", "startup", run_id, force=True)

            self.assertEqual(observed_phases, ["running"])
    ...
    async def test_v153_timeout_message_is_short_chinese_and_keeps_snapshot(self) -> None:
        state = REFRESH_STATE["news"]
        original = dict(state)
        state.update({"runId": 127, "version": 9})
        try:
            with (
                patch("src.background_refresh.STARTUP_STAGGER_SECONDS", 0),
                patch("src.background_refresh.execute_refresh", new=AsyncMock(side_effect=TimeoutError)),
            ):
                await run_refresh("news", "startup", 127, force=True)

            self.assertEqual(state["status"], "error")
            self.assertEqual(state["message"], "新闻刷新超时（180秒），已保留现有快照")
