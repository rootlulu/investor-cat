...
G10: 游戏页 → 用户可增删/导入关注游戏；Steam / 点点 / 七麦仅输出关注游戏；Sensor Tower 全球/中国 Top100 保留来源完整榜，关注目录仅补映射。
...
C40: Steam 在线/历史请求仅关注游戏；Steam/点点/七麦榜单可读取完整榜单以保留真实名次，但未关注行 ⊥ 进入业务缓存/API/UI；Sensor Tower 全球/中国 Top100 与同榜官方/媒体披露保留来源完整榜，关注目录仅补 `watchlistId`，⊥ 过滤/重编号。
...
V92: `games: []` 保存/重读仍空；Steam/点点/七麦 payload 均 0 关注行；Sensor Tower Top100 保持来源榜；⊥ DEFAULT_GAMES fallback。
V93: ∀ watchlist-scoped source row → sourceIds 精确命中 ∨ 唯一 alias 命中才保留；未命中/歧义 → 排除+可审计 warning；Sensor Tower Top100 按 C40 例外。
...
G11: `/ai#github` → 仅纳入 Stars 总数 ∨ 真实增长达标 AI 项目；ready/integrate 默认显示，专业 ML 训练框架、SDK、教程/合集默认隐藏；支持新上榜、近期爆发、关注与高信号提醒。
...
V101: discovery views = `recommended/new/rising/followed`；∀ view 候选先过 V160；recommended → Stars 总数降序；rising → 7 天、30 天 Stars 增长数降序；new → 创建时间降序；followed → 仅达标关注项。
...
V105: 卡片首要显示入榜依据、Stars 总数、7/30 天增长、task outcome、useStage/capability/surface、安装/Demo、release、关注/反馈；`discoveryScore` ⊥ UI；fork/language → 次要详情。
...
V160: ∀ GitHub AI 入榜项目 → `stars>=1000` ∨ (`historyStatus=ready` ∧ (`stars7dDelta>=100` ∨ `stars30dDelta>=500`))；`historyStatus=collecting` ⊥ 增长资格；`discoveryScore` ⊥ 入榜/排序/enrichment 优先级/UI；旧 schema 缓存读取时同门槛迁移。
...
T58|x|Stars 总数/真实增长硬门槛；Stars 主排序；移除综合分 UI；schema 7 缓存迁移+回归|V101,V105,V160,I.api.ai-projects,I.ui.ai-discovery
...
B116|2026-07-28|热门 search 门槛仅 20 Stars、recent 仅 5；`discoveryScore` 按主观可用性/新鲜度排序且无硬资格门槛 → 192 Stars+增长 `collecting` 排 #1|V160
...
V153: 新闻来源空结果 ≠ 抓取错误；`errors` ∀ 非空+来源+短原因；标题翻译有界超时 → 保留已抓新闻+warning；stale fallback 保留本轮异常；UI 摘要优先本轮失败。

## §T（新闻刷新异常提示）

id|status|task|cites
T56|~|分离新闻空结果/抓取错误；限制翻译阶段；短中文刷新提示；tests/build/API/browser|V28,V45,V48,V69,V153,I.api

## §B（新闻刷新异常提示）

id|date|cause|fix
B111|2026-07-28|Google News 63+ 同域请求受协调器串行限速，30s source timeout 含排队；标题翻译无总时限 → news 180s 超时；stale 状态先展示旧 Returns 空结果且 `TimeoutError` 丢来源|V153
B112|2026-07-28|多 warning 明细用 `；` 拼接 → 状态分段后把“标题翻译超时”脱离“来源提示”并误标异常|V153

## §G（Sensor Tower Top100）

G13: Sensor Tower 专区 → 最近完整月全球/中国手游预估流水 Top100；官方/权威媒体同口径披露优先；金额、估算方法、地区/渠道缺口可见。

## §C（Sensor Tower Top100）

C52: 来源优先级 = 官方披露 > 权威媒体披露 > Sensor Tower 授权 API/正式导出 > 公开 Sensor Tower 转述；仅同游戏+市场+月份合并，保留备选来源。
C53: 缺失地区/渠道/名次保持缺失；显示目标数、可用数、缺口数；⊥ 补 0、伪造 Top100、把移动端估算冒充全渠道流水。
C54: `SENSORTOWER_AUTH_TOKEN` 仅服务端读取；请求有界超时、≤2 市场调用；Token ⊥ 日志/API/UI。无有效授权 → 公开转述 fallback + 明确 `partial`。
C55: GACHAREVENUE 仅作公开 fallback；中国 Android = Sensor Tower 中国 iOS 估算 × 1.75，合计 = iOS × 2.75；PC/主机/广告收入与未覆盖地区 ⊥ 统计。

## §I（Sensor Tower Top100）

source.sensorTower.authorized: `GET https://api.sensortower.com/v1/unified/sales_report_estimates_comparison_attributes` → `WW|CN`、Games `6014`、latest complete month、limit `100`。
source.sensorTower.public: `revenue.ennead.cc/api` → session init + per-request signed token；公开 Sensor Tower 转述。
api.games.market: `GET /api/games` → `markets[].top100[]` + `coverage{status,targetCount,availableCount,missingCount,period,platforms,regions,excluded,method,sourceUrl}`。
ui.games.sensorTower: `/games` Sensor Tower 专区 → 精确金额 + 来源类型 + 统计月份 + 覆盖/缺口说明。

## §V（Sensor Tower Top100）

V127: valid `SENSORTOWER_AUTH_TOKEN` → ∀ `WW|CN` 请求 Games `6014` latest complete month limit `100`；401/403 → `configuration_required` + public fallback；Token ⊥ error/source metadata。
V128: public fallback auth → session init `POST /auth/token` + signed `GET` 含 `X-Request-Method`/`X-Nonce`；成功 → 全球 ≤100 + 中国实际可用数；⊥ watchlist 过滤。
V129: 同市场+月份游戏 identity 共享任一 appId/规范化中英文名 → 合并；优先级按 C52；原始候选进入 `revenueAlternatives`。
V130: ∀ market → `availableCount + missingCount = targetCount`；∀ row → 精确币种金额+method+period+coverage/excluded；中国公开 fallback 明示 `iOS + iOS×1.75 Android`。
V131: GACHAREVENUE 401 回归、官方 API 解析/脱敏、Top100 不过滤、官方覆盖、缺口披露测试 + 全量 tests + `npm run build` + live `/api/games` exit 0。

## §T（Sensor Tower Top100）

id|status|task|cites
T53|~|接入授权 API；修公开 fallback；完整 Top100；官方优先；金额/覆盖缺口 UI；验证|C40,C52-C55,V92,V93,V127-V131,I.source.sensorTower.*,I.api.games.market,I.ui.games.sensorTower

## §B（Sensor Tower Top100）

id|date|cause|fix
B92|2026-07-28|GACHAREVENUE 新鉴权需 session init + method + nonce；旧客户端 401 → 双榜 0|V128
B93|2026-07-28|Sensor Tower 榜套用 watchlist-only 过滤；完整 Top100 结构性不足|C40,V92,V93,V128

## §G（GitHub 关注库版本提醒）

G14: 已关注 GitHub 仓库发布稳定新版 → `/ai#github` 高信号区+系统通知可见。

## §C（GitHub 关注库版本提醒）

C56: 关注偏好仍仅存浏览器 localStorage；⊥ 上传项目/作者/用途关注。
C57: release/README enrichment 沿用匿名 ≤24、认证 ≤80 requests/refresh；轮转覆盖候选，失败保留 LKG。

## §I（GitHub 关注库版本提醒）

signal.release: `{eventId,type,projectKey,fullName,title,reason,occurredAt,url}`；stable release tag → 稳定 eventId。
ui.ai-release-alert: `/ai#github` 提醒区 → 普通高信号 + 直接关注项目稳定版；关注版优先。

## §V（GitHub 关注库版本提醒）

V132: `followedProjects` 命中 + stable release → 即使 `useStage ∈ {build,train_research,resource}`，提醒区+系统通知候选 ! 含该 release；owner/capability 宽泛关注 ⊥ 批量提升。
V133: enrichment 选择 → 未检查优先，其次 `enrichmentCheckedAt` 最旧优先；∀ attempt 更新时间；预算不变；失败保留 release/README LKG。
V134: 同 repo+release tag → 同 `eventId` 仅通知 1 次；新 tag → 新 `eventId`；draft/prerelease ⊥ 提醒。
V135: Python 轮转/identity/tag 回归 + Node 关注提升/去重/通知回归 + 全量 tests + `npm run build` + `git diff --check` exit 0。
V155: 单轮 unseen signals `N>0` → 仅 1 条汇总系统通知；成功 → `N` 个 eventId 全标 seen；constructor failure → 0 seen；同批 reload → 0 新通知。
V154: 提醒区 `N>4` → 默认明确显示 `4/N` + “查看全部 N 条”；展开 → ∀ N 可见；可收起，⊥ 静默截断。

## §T（GitHub 关注库版本提醒）

id|status|task|cites
T54|~|轮转检查候选 release；关注稳定版提升到提醒区/系统通知；单批汇总通知；提醒可展开全部；补回归与构建验证|C56,C57,V132-V135,V154,V155,I.signal.release,I.ui.ai-release-alert

## §B（GitHub 关注库版本提醒）

id|date|cause|fix
B94|2026-07-28|T54 red tests 复现 enrichment 固定前 12、release 信号无仓库 identity、前端无关注提升 helper|V132-V134
B95|2026-07-28|全量 pytest 被并行 T53 未完成实现+关注游戏清单 19<旧测试 25 阻断；本任务未触碰该链路|T53,V91,V127-V131
B113|2026-07-28|`notifyAiSignals` 对每个 unseen signal 各建 1 个 Notification → 单轮 18 条连续弹 18 次|V155
B114|2026-07-28|提醒列表固定 `slice(0, 4)` 且无剩余计数/展开入口 → 标题 18、正文仅 4|V154

## §V（AI 新闻标题翻译降级）

V136: ∀ AI 新闻标题翻译失败或旧 `translationStatus=fallback` → `title`=抓取原文 + `originalTitle`=抓取原文 + `translationStatus=original`；⊥ `<categoryLabel>最新动态`。

## §T（AI 新闻标题翻译降级）

id|status|task|cites
T55|x|保留 AI 新闻原始标题；跳过旧假标题缓存；补回归与验证|V136

## §B（AI 新闻标题翻译降级）

id|date|cause|fix
B96|2026-07-28|翻译失败 → 分类占位词覆盖真实标题；随后清空 `originalTitle`；旧 fallback 又被当中文缓存复用|V136

## §V（大宗库存市场隔离）

V150: ∀ 商品市场行 → 价格.market = 库存.market；国内库存 ⊥ 外盘 fallback；海外源缺失 → 明示“暂无对应海外库存”。
V151: 铜/铝/镍/锌/锡外盘库存 → LME 库存量+变动+日期+来源；历史无 market 快照 → 仅升级为 domestic，⊥ 复制到 international。
V152: Python 市场选择/解析回归 + Node 行级选择回归 + 页面五金属 DOM 核验 + `npm run build` + `git diff --check` exit 0。

## §B（大宗库存市场隔离）

id|date|cause|fix
B110|2026-07-28|`CommodityInventoryCell` 忽略 row、固定读取 `item.inventory`；后端仅暴露国内主库存 → 国内库存复制到外盘行|V150-V151

## §I（股票快照首屏与页内导览）

ui.stocks.jump: `/stocks` 大盘 → 吸顶章节轨道 `stock-market|stock-signals|stock-industry`；滚动/点击同步当前章节。

## §V（股票快照首屏与页内导览）

V156: `/api/stocks` 有效快照 → 大盘先独立落屏；`/api/stock-watchlist` 慢/失败 ⊥ 阻塞或清空市场/边际/机构数据。
V157: 首次快照未决 + ⊥ 已解析数据 → 显示“正在读取最近快照”骨架；⊥ “暂未取到/建立首个快照/本轮未取到”空态。
V158: 大盘章节轨道 → 桌面位于全局 topbar 下方吸顶；移动端 top=`8px`；当前章节高亮；锚点 ⊥ 被导航遮挡；键盘焦点可见。
V159: Node 延迟自选回归 + `npm run build` + 1440/390 browser QA → 快照先显、吸顶位置稳定、滚动高亮、zero console errors。

## §T（股票快照首屏与页内导览）

id|status|task|cites
T57|x|拆分股票/自选请求；修首次加载语义；重做吸顶章节轨道；tests/build/API/browser|V31,V61,V69,V156-V159,I.ui.stocks.jump

## §B（股票快照首屏与页内导览）

id|date|cause|fix
B115|2026-07-28|`StocksPage.loadStocks` 串行等待股票快照后再读自选，且两者完成前用空对象渲染真实缺失文案；首次自选读取 3456ms → 已有股票快照仍显示全页无数据|V156-V157

## §V（股票刷新异步编排）

V162: `get_stocks(force=true)` → 中国/世界行情 + 市场概览/行业融资/机构持仓同时启动；边际信号仅依赖市场概览完成；并发任务集合固定有界；同域请求仍受 `RequestState`/`REQUEST_COORDINATOR` 限流。
V163: 任一股票来源异常 ⊥ 取消其他来源；各分项独立回退 empty/LKG；响应 schema、诊断、`save_latest_stocks` 条件不变；调用取消继续向上传播。

## §T（股票刷新异步编排）

id|status|task|cites
T59|x|股票刷新固定任务有界并发；依赖顺序/失败隔离回归；全量 tests/build/diff check|V61,V69,V162-V163,I.api

## §B（股票刷新异步编排）

id|date|cause|fix
B117|2026-07-28|FastAPI handler 内独立外部 I/O 仍逐个 `await`/`to_thread`，刷新总耗时近似各来源耗时之和|V162-V163
