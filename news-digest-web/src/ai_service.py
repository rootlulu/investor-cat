...
async def fetch_ai_news_snapshot(ttl_seconds: int, previous: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    for item in selected:
        ...
        if previous_item and has_chinese(previous_item.get("title", "")):
            item["title"] = previous_item["title"]
            item["summary"] = previous_item.get("summary", "") if has_chinese(previous_item.get("summary", "")) else ""
            item["translationStatus"] = "cached"
        elif not has_chinese(item.get("title", "")):
    ...
    for item in selected:
        if not has_chinese(item.get("title", "")):
            translated = translations.get(item["title"], "")
            if translated:
                item["title"] = translated
                item["translationStatus"] = "translated"
            else:
                item["title"] = f"{item.get('categoryLabel') or 'AI'}最新动态"
    ...
    return {
        ...
        "errors": list(dict.fromkeys([*errors, *( [f"{untranslated_count}条标题使用中文兜底"] if untranslated_count else [])]))[:20],
        "items": selected,
