...
async def fetch_public_sensor_tower_revenue(timeout: float) -> tuple[list[dict[str, Any]], list[str], dict[str, Any] | None]:
    ...
    source = {
        ...
        "note": "公开网页转述的 Sensor Tower 估算，覆盖偏二游/抽卡游戏，仅作无授权时趋势兜底。",
    }
    return rows, [], source


async def gacharevenue_api(client: httpx.AsyncClient, path: str) -> dict[str, Any]:
    client_token = str(uuid.uuid4())
    request_path = f"/{path}"
    auth_headers = {"X-Client-Token": client_token, "X-Request-Path": request_path}
            ...
            clean_dict(
                {
                    ...
                    "note": f"公开 SensorTower 估算转述；覆盖区域：{', '.join(sorted(row.get('regions') or []))}",
                }
            )
        )
    return result
...
def build_markets(
    ...
    for definition in MARKET_DEFS:
        ...
        markets.append({**definition, "rankLimit": rank_limit, "top100": top100, "rowCount": len(top100)})
...
def preferred_revenue_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for index, row in enumerate(rows):
        key = revenue_group_key(row, index)
...
def build_provider_status(
    ...
    sensor_status = "imported" if revenue_files else ("public_fallback" if sensor_rows else ("credentials_present" if token_present else "needs_credentials_or_export"))
    sensor_message = (
        f"已读取 {len(sensor_rows)} 条 Sensor Tower 正式导出流水。"
        if revenue_files
        else (
            f"已读取 {len(sensor_rows)} 条公开 Sensor Tower 估算转述；覆盖偏二游/抽卡游戏，正式导出会自动覆盖。"
            if sensor_rows
            else f"未发现 Sensor Tower 导出文件；可配置 {token_env} 或导入 data/game_sensor_tower_revenue.csv/json。"
        )
    )
    ...
    statuses = [
        ...
        },
        {
            ...
            "sourceFiles": revenue_files or public_revenue_files,
            "homeUrl": GACHAREVENUE_SOURCE_URL if public_revenue_files and not revenue_files else "https://app.sensortower.com/",
