...
`/ai#github/recommended` 不再按固定 GitHub 类别各取 Stars Top 30，而是把项目拆成“使用门槛、用途、
产品形态”三个维度。默认只显示“直接可用”和“安装即用”，开发组件、训练研究和教程资料须由用户显式
放出。固定发现视图包括“为你推荐、新上榜、近期爆发、已关注”；旧 `#github/skills`、`mcp` 等链接会
在前端迁移为等价筛选。
...
`start-dev.sh`、Vite dev server 和 `npm run preview` 默认仅监听 `127.0.0.1`。确需在局域网提供只读页面时，
可以显式执行 `HOST=0.0.0.0 ./start-dev.sh`。Vite 会把实际客户端地址转发给后端；远程 `POST`、`PUT`、
